...
int main(void)
{
 ...
 do { struct test_item *p; p = (struct test_item*)malloc(sizeof(struct test_item)); p->name = "char"" to ""int"; (int*)p->buf = (char)15; p->len = sizeof(int); menu[item_counter++] = p; }while(0);
# 54 "main.c"
 int j;

 for (j = 0; j < sizeof(menu)/sizeof(menu[0]); i++) {
  printf("----------%s--------\n", menu[j]->name);
  show_bytes(menu[j]->buf, menu[j]->len);
  printf("--------------------\n");
 }
