...
/* add by liupeng start
 */
#include "nt_kernel.h"
#include "ruijie.h"
#define add_log(fmt,args...) do{ 		\
	scall_param pa={0},pb={0},pc={0};	\
	char buf[2048];				\
	snprintf(buf, sizeof(buf), fmt, ##args);	\
	ruijie_SwitchCall(RUIJIE_LOG_ADD, PRM_POINTER(pa, buf, sizeof(buf)), PRM_VALUE_NULL(pb), PRM_VALUE_NULL(pc)); \
}while(0)
/* end*/
...
static int ruijie_call_back(int reason,const char * current_packet,void*userptr)
{
  ...
  switch(reason)
  {
	...
	case RUIJIE_AUTH_NEEDNAME:
	  ...
	  add_log("查找服务器成功，使用%s登陆 ......", sender.m_name);
	  break;
